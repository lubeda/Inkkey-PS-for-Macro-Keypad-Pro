#region Keycode definitions
$KeyboardMods = @{
    MOD_NONE        = 0
    MOD_LEFT_CTRL   = (1 -shl 8)
    MOD_LEFT_SHIFT  = (1 -shl 9)
    MOD_LEFT_ALT    = (1 -shl 10)
    MOD_LEFT_GUI    = (1 -shl 11)
    MOD_RIGHT_CTRL  = (1 -shl 12)
    MOD_RIGHT_SHIFT = (1 -shl 13)
    MOD_RIGHT_ALT   = (1 -shl 14)
    MOD_RIGHT_GUI   = (1 -shl 15)
}
...
#New-Variable -Name "mkport" -Scope Script -Description "The open port"
$script:portname = $null

function start-MKPort {
    [CmdletBinding()]
    param (
        [Parameter()]
        [String]
...
function Set-MKkey {
    ...
    begin {
        if ($script:portname) {
            Write-Verbose "Using $($script:portname)"
        }
        else {
            Write-Error "No Port set use ""Set-MKPort"""
        }
    }
    ...
    process {
        ...
        if ($myport.BytesToRead -gt 0) {
            $myport.readExisting()
        }
        stop-MKPort -port $myport
    }
}

function Get-MKsequenceStep() {
    [CmdletBinding()]
    param (
...
function Set-MKPort {
    ...
    begin {
        Get-CimInstance -ClassName Win32_PnPEntity | Where-Object { $_.Name -like "*(COM*)*" } | Write-Verbose
        Write-Debug "using $portname"
    }
    
    process {
        $script:portname = $portname
    }
    end {
    }
}

Export-ModuleMember -Variable $KeyboardKeycode 
Export-ModuleMember -Function Set-Mkport
Export-ModuleMember -Function Start-Mkport
